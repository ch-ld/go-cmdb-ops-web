<template>
  <div class="app-container">
    ...
    <div class="custom-dialog" v-if="connectionOptionsDialog">
      <h2>请选择连接方式</h2>
      <label>
        <input type="radio" v-model="selectedConnectionType" value="PASSWORD" /> 密码连接
      </label>
      <label>
        <input type="radio" v-model="selectedConnectionType" value="PUBLICKEY" /> 密钥连接
      </label>
...
export default {
  ...
  methods: {
    async fetchServers() {
      try {
        const response = await getCmdb({ id: 0 });
