...
export default {
  ...
  methods: {
    ...
    // 编辑记录
    editRecord(record) {
      console.log('Editing record', record) // 通过 console 检查是否触发
      // 深拷贝记录，确保不会共享引用
      this.currentRecord = {
        id: record.id,
        title: record.title,
        desc: record.desc,
        logo: record.logo,
        // 确保 links 数组在每次编辑时是独立的
        links: record.links ? [...record.links] : [{ name: '', url: '' }] // 如果 record.links 为空，初始化为空数组
      }
      this.visible = true // 显示弹窗
    },
    ...
    async saveRecord() {
      try {
        console.log('Saving record', this.currentRecord) // 检查当前的数据
    ...
    async deleteRecord(id) {
      try {
        ...
        if (error === 'cancel') {
          this.$message.info('已取消删除') // 如果点击了取消，显示提示信息
        } else {
    ...
    async sortRecord(id, direction) {
      try {
        console.log('Sorting record with ID:', id, 'Direction:', direction)
        const payload = { id, direction } // 构造请求的负载
        await sortNavigationRecord(payload) // 发送数据
