...
export default {
  data() {
    return {
      ...
      visible: false // 表单弹窗显示状态
    }
  },

  mounted() {
    this.fetchRecords() // 初始化时获取记录
  },
  ...
  methods: {
    ...
    // 编辑记录
    editRecord(record) {
      this.currentRecord = { ...record } // 复制记录数据
      this.visible = true // 打开表单弹窗
    },
    ...
    // 保存记录
    async saveRecord() {
      if (!this.currentRecord.title || !this.currentRecord.desc || !this.currentRecord.logo) {
      ...
      try {
        if (this.currentRecord.id) {
          await updateNavigationRecord(this.currentRecord) // 更新记录
        } else {
    ...
    async deleteRecord(id) {
      try {
        await deleteNavigationRecord(id) // 删除记录
    ...
    async sortRecord(id, direction) {
      try {
        await sortNavigationRecord(id, direction) // 排序记录
