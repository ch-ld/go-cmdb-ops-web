...
import GroupTree from '@/views/host/GroupTree.vue'
import HostForm from '@/views/host/HostForm.vue'
import HostSettings from '@/views/host/HostSettings.vue'
  ...
  setup() {
    ...
    const handleValidate = async() => {
      if (selectedHosts.value.length === 0) {
        this.$message.warning('请选择要验证的主机')
        return
      }
      try {
        loading.value = true
        await validateHosts(selectedHosts.value.map(host => host.id))
    ...
    const handleSelectionChange = (selection) => {
      ...
      isAllSelected.value = selection.length === filteredHosts.value.length
    }
